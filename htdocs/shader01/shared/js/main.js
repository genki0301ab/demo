...
Stage.prototype.createObject = function() { //createObject
    ...
    this.material = new THREE.ShaderMaterial({
        ...
        vertexShader: this.gl.element.$fs[0].innerHTML,
        fragmentShader: this.gl.element.$vs[0].innerHTML
    });
...
function Uniform(Object) {
    ...
    this.uniforms = {
        ...
        resolution: {
            type: "v2",
            value: new THREE.Vector2(this.gl.window.width, this.gl.window.height)
        },
        mouse:  {
            type: "v2" ,
        ...
        texture_perlin: {
            type: "t",
            value: new THREE.TextureLoader().load("shared/img/perlin.jpg")
        }
    };
    this.range = {
        min: 0.0,
...
Uniform.prototype.normalize = function(domain) { //normalize
    var result = d3.scaleLinear().domain([d3.min(domain), d3.max(domain)]).range([this.range.min, this.range.max, this.range.max]);
    return result;
}
Uniform.prototype.sendShader = function() { //sendShader
    var normalizeAudio = this.normalize(this.gl.audio.frequencyData);
    this.gl.uniform.uniforms.audio.value = normalizeAudio(this.gl.audio.frequencyData[0]);
    this.gl.uniform.uniforms.time.value += 0.001;
};
...
function Audio(Object) {
    ...
    //property
    this.current = 0;
    this.file = ["shared/audio/audio.m4a"];
    this.fftSize = 2048;
